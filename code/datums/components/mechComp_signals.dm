...
/datum/component/mechanics_holder/RegisterWithParent()
	RegisterSignal(parent, list(COMSIG_MECHCOMP_ADD_INPUT), PROC_REF(addInput))
	RegisterSignal(parent, list(_COMSIG_MECHCOMP_RECEIVE_MSG), PROC_REF(fireInput))
	RegisterSignal(parent, list(COMSIG_MECHCOMP_TRANSMIT_SIGNAL), PROC_REF(fireOutSignal))
	RegisterSignal(parent, list(COMSIG_MECHCOMP_TRANSMIT_MSG), PROC_REF(fireOutgoing))
	RegisterSignal(parent, list(COMSIG_MECHCOMP_TRANSMIT_DEFAULT_MSG), PROC_REF(fireDefault)) //Only use this when also using COMSIG_MECHCOMP_ALLOW_MANUAL_SIGNAL
	RegisterSignal(parent, list(_COMSIG_MECHCOMP_RM_INCOMING), PROC_REF(removeIncoming))
	RegisterSignal(parent, list(_COMSIG_MECHCOMP_RM_OUTGOING), PROC_REF(removeOutgoing))
	RegisterSignal(parent, list(COMSIG_MECHCOMP_RM_ALL_CONNECTIONS), PROC_REF(WipeConnections))
	RegisterSignal(parent, list(_COMSIG_MECHCOMP_GET_OUTGOING), PROC_REF(getOutgoing))
	RegisterSignal(parent, list(_COMSIG_MECHCOMP_GET_INCOMING), PROC_REF(getIncoming))
	RegisterSignal(parent, list(_COMSIG_MECHCOMP_DROPCONNECT), PROC_REF(dropConnect))
	RegisterSignal(parent, list(_COMSIG_MECHCOMP_LINK), PROC_REF(link_devices))
	RegisterSignal(parent, list(COMSIG_MECHCOMP_ADD_CONFIG), PROC_REF(addConfig))
	RegisterSignal(parent, list(COMSIG_MECHCOMP_ALLOW_MANUAL_SIGNAL), PROC_REF(allowManualSingalSetting)) //Only use this when also using COMSIG_MECHCOMP_TRANSMIT_DEFAULT_MSG
	RegisterSignal(parent, list(COMSIG_ATTACKBY), PROC_REF(attackby))
	RegisterSignal(parent, list(_COMSIG_MECHCOMP_COMPATIBLE), PROC_REF(compatible))//Better that checking GetComponent()?
	return  //No need to ..()
