	...
	attackby(obj/item/I, mob/user)
		if(isweldingtool(I))
			...
			if(!inserted)
				...
				SETUP_GENERIC_ACTIONBAR(user, src, 5 SECONDS, PROC_REF(insert_module_callback), list(count,user,AIM), user.equipped().appearance, null, \
			...
			if (GET_COOLDOWN(src, "mine_cooldown") == 0)
				SETUP_GENERIC_ACTIONBAR(user, src, 5 SECONDS, PROC_REF(insert_videocard_callback), list(user,V), user.equipped().appearance, null, \
						"", INTERRUPT_ACTION | INTERRUPT_MOVE | INTERRUPT_STUNNED | INTERRUPT_ACT)
			else
		...
		switch(action)
			if("weld")
				...
				if(!equipped:try_weld(ui.user, 1, burn_eyes = 1, noisy = 2))
					...
					actions.start(new /datum/action/bar/private/welding(ui.user, src, 5 SECONDS, PROC_REF(toggle_welded_callback), list(slotNum,ui.user), \
			...
			if("screw")
				...
				SETUP_GENERIC_ACTIONBAR(ui.user, src, 5 SECONDS, PROC_REF(toggle_screwed_callback), list(slotNum,ui.user), ui.user.equipped().icon, ui.user.equipped().icon_state, \
				"", INTERRUPT_ACTION | INTERRUPT_MOVE | INTERRUPT_STUNNED | INTERRUPT_ACT)
			...
			if("rack")
				...
				if(law_circuits[slotNum])
					...
					SETUP_GENERIC_ACTIONBAR(ui.user, src, 5 SECONDS, PROC_REF(remove_module_callback), list(slotNum,ui.user), law_circuits[slotNum].icon, law_circuits[slotNum].icon_state, \
					"", INTERRUPT_ACTION | INTERRUPT_MOVE | INTERRUPT_STUNNED | INTERRUPT_ACT)
				else
					...
					SETUP_GENERIC_ACTIONBAR(ui.user, src, 5 SECONDS, PROC_REF(insert_module_callback), list(slotNum,ui.user,equipped), ui.user.equipped().icon, ui.user.equipped().icon_state, \
					"", INTERRUPT_ACTION | INTERRUPT_MOVE | INTERRUPT_STUNNED | INTERRUPT_ACT)
